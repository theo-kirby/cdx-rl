...
| 4b | [`flywheel-conventions.md`](flywheel-conventions.md) | **How we use Flywheel: the four lanes, protocol→result→replication, the tag vocabulary, retractions, the 4 KB body budget, and how to fork a graph. Normative — read before writing any node.** §13 is the v2 node table. |
...
| Docs | ✅ this set |
| Flywheel | ✅ **v2 root `steep-pine-4944` = `91c66efb-66f3-48aa-b7c1-14bfe00bb09f`, FORTY-TWO nodes, forked and restructured 2026-08-06.** Four lanes off the root — **research / instrument / substrate / platform** — replacing v1's single 9-node spine. Research is shaped **protocol → result → replication**, so ADR-097 is structural: a protocol node is written *before* dispatch and "what is in flight" is the query `type/protocol AND status/planned`. 55 artifacts carried, 40 edges, 4 multi-parent nodes, and **0 bodies over 4 KB** (v1 had 11, so eleven nodes could not be corrected inside a 60 s lease). New in v2 and absent from v1 entirely: the **five merged Cadex PRs**, the **driver register**, the **torque-instrument defect**, the **GPU ledger**, and **experiment 005-ceiling** — a finished 4.75 h run that had left no trace. **Tags: 27 created, 125 assignments over 36 nodes** (the count leaves six untagged where only five are the root and the lanes — the arithmetic was already off by one at 39/33 and which node it is has not been chased). **`status/planned` is OCCUPIED, by `winter-math-2265` = `9cf7c9ec-99df-5ca6-8550-5087e3a56237` (PROTOCOL 007)** — its Phase A is measured on CPU and has a result child (`curly-hat-3297`, with the audit attached as a `table`), its Phase B is not written, and **no GPU is committed to it**. That is exactly what the in-flight query is for. Before it, `wandering-cell-5009` (**PROTOCOL 006**) took the tag before any of its 9.67 GPU-hours were spent and flipped to `status/resolved` when its result child `soft-field-3236` landed — **the first time the graph gated a run rather than recording it afterwards.** New 2026-08-07: 007's protocol, its Phase A result, and `holy-art-8641` (the `reward_audit` driver + its basename defect, in the instrument lane with a second parent into the result). There is **no `task/` namespace** — one task partitions nothing. `flywheel-conventions.md` §5 has the tag ids and the two mechanical traps (`create_node_tag` takes the **graph** revision and serialises; `set_node_tag_assignments` takes the **node's** and parallelises), §13 the per-node table. **v1 `rapid-bar-6214` is FROZEN and must not be deleted**: v2's artifact blobs are aliases into its storage. Conventions in [`flywheel-conventions.md`](flywheel-conventions.md) |
...
| | ✅ **`mechanisms/mg-legs/drivers/reward_audit.py`, new 2026-08-07** — which reward terms actually **shape** behaviour at the states a policy reaches, as the standard deviation of `w · kernel` in absolute reward units beside the share of its own weight the term collects. **The spread alone cannot be read**: `achieved` (the policy won the term) and `dead` (pinned low, unreachable) are the same standard deviation and opposite findings, so the share column separates them. Its self-check asserts that `alive` — the expression `1` — has spread exactly zero. Trainer venv, never `uv run`; pure half tested at **23 checks**, no mujoco. 5.7 s for 6 rows × 12 seeds |
...
| Experiment 006 | ✅ **two seeds, 9.67 GPU-h, both `rc 0` — and the headline REPLICATES while the FALSIFIER FIRES.** *Does costing joint velocity make it stand still?* **Yes, twice: settled `Σ|q̇|` falls **−69.4 %** (seed 2) and **−53.9 %** (seed 1) against the matched `stand12` controls, on a quantity no existing kernel could see. Command jitter falls 17.5° → 5.8° per step. **But Q3 — the pre-registered bracing falsifier — does NOT replicate: seed 2 rests at 0.98 % duty and 38.6 % of rating (better than the control's 13.88 % / 53.5 %), seed 1 at 45.19 % and 73.4 % (WORSE than the control), against a 25 % ceiling.** Same bundle, same hyperparameters, same trainer; only the seed differs. **`quiet` does not say HOW to be still** — holding a joint still by not commanding motion and by stiffening against the servo are the same reading on `Σ|q̇|` and opposite readings on `actuator_force`, and PPO found both basins. **Not adoptable as written.** Q2 passes both (p = 1.000, p = 0.125) but seed 1's conjunction fell 15/24 → 10/24, so Q2 failed to *detect* a loss rather than showing none |
| Experiment 007 | 🟡 **Phase A measured on CPU in 5.7 s, ZERO GPU. Phase B is NOT WRITTEN and nothing may be dispatched until it is.** *Does the reward price bracing at the states the machine actually reaches?* **The pre-registered `dead` verdict did NOT fire** — `effort` returns `sd_paid` 0.020–0.032 against a 0.01 floor, so it has gradient. What is true instead: **it is priced trivially.** It collects **0.10–0.14 of its own weight** where every other shaping term collects 0.47–0.99, it steers **2.5–3.0 %** of the total spread against `capture`'s ~39 %, and it is **the only term a trained policy collects LESS of than the zero-action servo does** (0.41 → 0.10). **And it ranks 006's two basins correctly for 0.69 % of the budget**: the still seed (0.98 % duty) and the bracing seed (45.19 %) differ by **0.0368 of a 5.30 budget**, and the bracing seed still took home 88.2 % of the total. The sign is right; the magnitude is not. **Correlational, five policies, one bundle, one scoring seed set.** `experiments/007-price-the-bracing/README.md` |
...
**Total GPU-hours: 49.32 measured in `jobs/` on sb1x**, plus ~7.3 off-box on sb9x (not independently verifiable here — those run directories are not in this repository).

> **Re-summed 2026-08-07 after experiment 006: 49.32 h.** The 39.66 figure was correct when written and 006 added **9.67 h** (`stand15` s2 4.86 + s1 4.80, both `rc 0` at iteration 1799). It is still a **hand-run sum over `jobs/*/progress.json` where `device == "gpu"`**, and it will drift again until a driver writes it — which is now the third time this has been said.
>
> **The old figure was ~27.5 and it was wrong.** Summing `wall_time_s` over every `jobs/*/progress.json` with `device == "gpu"` on 2026-08-06 gives **39.66 h**. The ledger below omitted **experiment 004 entirely** (10.12 h across three arms), **the seed-1 replication** (4.87 h) and **stand13 / 005-ceiling** (4.75 h); its "002 seeds 0 and 1 — ~5.1" is 4.24 h measured. Two of the omissions are runs that had no experiment node either. The graph now carries a **GPU-ledger node** in the platform lane with the per-run table — but nothing regenerates it, so **it is a hand-run sum and will drift again until a driver writes it.**
...
* **`harness steps` keys `results` by the policy's BASENAME, so two seeds of
  ...
  **And the same defect shipped again in `reward_audit.py` on 2026-08-07** —
  its result rows were keyed by basename too, so `stand15` seeds 1 and 2 were
  distinguishable only by row order. Nothing was summed there, so it was
  *unreadable* rather than *wrong*, which is why it survived review. Fixed:
  rows carry `<run dir>/<file>`, the resolved `policy_path` and the `run`, the
  file records the `task` and `model` it scored under, and
  `check_labels_unique` refuses the same file twice. **The pattern is worth
  more than either instance: the identifier that is convenient to PRINT is not
  the identifier that IDENTIFIES the thing.** Check it in any new driver that
  takes repeated `--policy`.
